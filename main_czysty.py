    ...
    def log_matrix(self, title):
        if self.allocation is not None and self.basic_vars is not None:
            ...
            self.step_by_step_data.append({
                ...
                'pretty_table_data': pretty_table_data,
                'u': u,
                'v': v
            })
...
class TransportApp:
    ...
    def generate_table(self):
        ...
        ttk.Label(self.table_frame, text=r"Dost\Odb").grid(row=0, column=0)
        for j in range(cols):
    ...
    def format_step_table(self, data, step_index):
        ...
        header_parts = [f"{r'Dost\Odb':<{SIDE_WIDTH}}"]
        for j in range(cols):
